public class FenwickTreeRangeUpdatePointQuery {
    ...
    public FenwickTreeRangeUpdatePointQuery(long[] values){
        if(values==null)throw new IllegalArgumentException("Values of array cannot be null!");
    ...
    //Get the value at a specific index. The logic behind this method is the same as finding the prefix sum in a Fenwick tree except that you need to take the difference between the current tree and the original to get the point value.
