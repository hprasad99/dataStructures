...
public abstract class HashTableOpenAddressingBase<K, V> implements Iterable<K> {
    ...
    // tracks the number of unique keys current;y inside the hash-table
    ...
    private static double DEFAULT_LOAD_FACTOR = 0.65;
    ...
    // Increases the capacity of hash table.
    ...
    // Returns the number of keys currenty inside the hash-table
    public int size() {
        return keyCount;
    }
    ...
    protected void resizeTable() {
        ...
        // Reset the key count and buckets used since we are about to re-insert all the
        // keys
        keyCount = usedBuckets = 0;
    ...
    public V insert(K key, V val) {
        ...
        for (int i = offset, j = -1, x = 1;; i = normalizeIndex((offset + probe(x++)))) {
            ...
            if (keys[i] == TOMBSTONE) {
                ...
                // Current cell is null so as insertion/update can occur
            } else {
                ...
                if (j == -1) {
                    ...
                    // where the null element is insert
    ...
    public boolean hasKey(K key) {
        ...
        for (int i = offset, j = -1, x = 1;; i = normalizeIndex(offset + probe(x++))) {
            ...
            // of a deleted cell is found to peform lazy relocation later.
            if (keys[i] == TOMBSTONE) {
                if (j == -1)
                    j = i;
                ...
                if (keys[i].equals(key)) {
                    ...
                    // i and j so that the next time we search for this key it will be found faster.
    ...
    public Iterator<K> iterator(){
        ...
        // otherwise a concurrent modification has occured :0
        ...
        return new Iterator<K>(){
            ...
            public boolean hasNext(){
                //The contents of the table have altered
