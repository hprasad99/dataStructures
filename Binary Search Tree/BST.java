public class BST<T extends Comparable<T>> {
    ...
    private Node remove(Node node, T elem) {
        ...
        if (cmp < 0) {
            ...
            //
            // Dig into right subtree, the value we're looking for is smaller than the
            // current value
        } else if (cmp > 0) {
            ...
            if (node.left == null) {
                ...
                // data with. This prevents is from having two nodes in our tree with the same
                // value
                node.right = remove(node.right, tmp.data);
                // If instead we wanted to find the largest node in the left subtree as opposed
                // to smallest node in the right subtree
    ...
    // Helper method to find the righmost node (which has the largest value)
