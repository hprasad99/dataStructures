public class AVLTreeRecursiveOptimized<T extends Comparable<T>> implements Iterable<T> {
    ...
    // we can return the TOKEN instead of null when we're inserting a new item and
    // discover the value we are inserting already exists in the tree.
    private Node TOKEN = new Node(null);

    // The height of a rooted tree is the number of edges between the tree's
    // root and its furthest leaf. This mean that a tree containing a single node
    // has a height of 0.
    public int height() {
        if (root == null)
            return 0;
    ...
    public boolean insert(T value) {
        ...
        boolean insertNode = (newRoot != TOKEN);
        if (insertNode) {
            nodeCount++;
            root = newRoot;
        }
    ...
    // Insert a value inside the AVL tree. This method returns 'TOKEN' if the value
    // we tried to insert was already inside the tree, otherwise the new(or old)
    ...
    //Removes a value from the AVL tree. If the value we're trying to remove does not exists in the tree then the 'TOKEN' value is returned. 
    //Otherwise, the new(or old) root node is returned.
    private Node remove(Node node,T elem){
        ...
        if(cmp<0){
            ...
            if(node.left == null){
                ...
                //When removing a node from binary tree with two links the successor of the node being removed can either be the largest value in the left subtree
                //or the smallest value in the right subtree. As a heuristic, I will remove from  the subtree with the most index in hopesthat this may help with balancing.
            }else{
